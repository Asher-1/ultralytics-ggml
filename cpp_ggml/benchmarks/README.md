...
- [latency_matrix.png](latency_matrix.png): all 11 models across all three GGML backends and all three precisions.
...
| CUDA faster than PyTorch CUDA for every model  | cuDNN 9.1 snapshot: F16 passes 11/11 models, x1.04 to x2.01                                              |
| Vulkan close to PyTorch CUDA for every model   | Snapshot passes the declared x0.70 throughput floor: 11/11 models, x0.70 to x1.32                        |
...
The optimized CUDA configuration intentionally retains the measured cudnn-frontend v1.8.0 dependency, whose minimum
supported backend is cuDNN 8.5. Older cuDNN installations can use the generic CUDA build, but that compatibility path is
not performance-certified and must not be relabeled as the cuDNN 9.1 snapshot above.
