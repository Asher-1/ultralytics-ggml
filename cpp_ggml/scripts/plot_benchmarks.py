...
def validate_coverage(bench_records, torch_records, seg_records, seg_torch_records):
    ...
    expected_seg = {(backend, model, dtype) for backend in SEG_BACKENDS for model in SEG_MODELS for dtype in DTYPES}
    missing_seg = sorted(expected_seg - set(seg_records))
    missing_seg_torch = sorted({(model,) for model in SEG_MODELS} - set(seg_torch_records))
    ggml_sections = {
    ...
    for key in sorted(expected_seg & set(seg_records)):
        missing = require_metrics(seg_records[key], ggml_sections)
        if missing:
            incomplete.append(f"ggml-seg {key}: {', '.join(missing)}")
    ...
    for key in sorted({(model,) for model in SEG_MODELS} & set(seg_torch_records)):
        missing = require_metrics(seg_torch_records[key], torch_sections)
        if missing:
            incomplete.append(f"pytorch-seg {key}: {', '.join(missing)}")
    problems = []
    if missing_ggml:
    ...
    if missing_seg:
        problems.append(f"missing ggml seg keys ({len(missing_seg)}): {missing_seg}")
    if missing_seg_torch:
        problems.append(f"missing PyTorch seg keys ({len(missing_seg_torch)}): {missing_seg_torch}")
...
def main():
    ...
    seg_records = {}
    for seg_file in ("seg_cuda.jsonl", "seg_vulkan.jsonl"):
        seg_records.update(load(RES / seg_file, lambda d: (d["backend"], d["model"], d["dtype"])))
    seg_torch_records = load(RES / "pytorch_seg.jsonl", lambda d: (d["model"],))
    validate_coverage(bench_records, torch_records, seg_records, seg_torch_records)
    ...
    seg = {key: (record["e2e_ms"]["mean"], record["backend"]) for key, record in seg_records.items()}
    segref = {key: (record["e2e_ms"]["mean"], record["backend"]) for key, record in seg_torch_records.items()}
    ...
    # ---- Chart 6: segmentation e2e by model scale (cuda/vulkan vs PyTorch) ----
    seg_series = [
        ("PyTorch CUDA (f32)", "pytorch-cuda", "#d62728"),
        ("ggml CUDA", "cuda", "#1f77b4"),
        ("ggml Vulkan", "vulkan", "#2ca02c"),
    ...
    for ax, family in zip(axes, ["yolov8", "yolo26"]):
        ...
        x = range(len(models))
        width = 0.8 / len(seg_series)
        center = (len(seg_series) - 1) / 2
        for i, (label, tag, color) in enumerate(seg_series):
            vals = []
            for m in models:
                if tag == "pytorch-cuda":
                    vals.append(segref.get((m,), (None,))[0])
                else:
                    dt = (
                        "f16"
                        if (tag, m, "f16") in seg
                        else next((dt for dt in ("f32", "q8_0") if (tag, m, dt) in seg), None)
                    )
                    vals.append(seg.get((tag, m, dt), (None,))[0])
            bars = ax.bar([xi + (i - center) * width for xi in x], plot_values(vals), width, label=label, color=color)
            for b, v in zip(bars, vals):
                if v:
                    ax.text(b.get_x() + b.get_width() / 2, v + 0.3, f"{v:.1f}", ha="center", va="bottom", fontsize=7)
        ax.set_xticks(list(x))
        ax.set_xticklabels([m.replace(family + "-seg", "") for m in models])
        ax.set_yscale("log")
        ax.set_title(f"{family}-seg — end-to-end latency on bus.jpg (lower is better)")
        ax.set_ylabel("ms / frame (log scale)")
        ax.legend(fontsize=8)
        ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    ...
    for m in SEG_MODELS:
        pt = segref.get((m,), (None,))[0]
        for dt in dtypes:
            row = [m, dt, f"{pt:.2f}" if pt else "-"]
            for tag in ("cuda", "vulkan"):
                v = seg.get((tag, m, dt), (None,))[0]
                row += [f"{v:.2f}" if v else "-", f"x{pt / v:.2f}" if (v and pt) else "-"]
            lines.append("| " + " | ".join(row) + " |")
    (RES / "speedup_table.md").write_text("\n".join(lines) + "\n")
    print(
        "validated 99 GGML + 11 PyTorch detect keys and 60 GGML + 10 PyTorch seg keys; "
        "wrote speed_by_model.png, latency_by_backend.png, speed_by_dtype.png, depth_latency.png, "
        "latency_matrix.png, seg_latency.png, speedup_table.md"
    )
